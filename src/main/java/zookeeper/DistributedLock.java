...
public class DistributedLock implements Watcher{
    ...
    //确保连接zk成功；
    private CountDownLatch connectedSemaphore = new CountDownLatch(1);//CountDownLatch.await()等待指定数量的线程执行完成
    //确保所有线程运行结束；
                ...
                public void run() {
                    try{
                        ...
                        synchronized (threadSemaphore){
                            dc.createPath(GROUP_PATH, "该节点由线程" + threadId + "创建", true);//创建父Znode
                        }
                        dc.getLock();
                    } catch (Exception e){
    ...
    private void getLock() throws KeeperException, InterruptedException {
        selfPath = zk.create(SUB_PATH,null, ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL_SEQUENTIAL);//创建子Znode
    ...
    public void createConnection( String connectString, int sessionTimeout ) throws IOException, InterruptedException {
        ...
        connectedSemaphore.await();//等待zookeeper创建完成
    }
    /**
     * 获取锁成功
     */
    public void getLockSuccess() throws KeeperException, InterruptedException {
        ...
        zk.delete(this.selfPath, -1);//删除节点Znode
        releaseConnection();//释放链接
    ...
    public boolean checkMinPath() throws KeeperException, InterruptedException {
        ...
        switch (index){
            ...
            default:{
                this.waitPath = GROUP_PATH +"/"+ subNodes.get(index - 1);//前一个节点路径
                LOG.info(LOG_PREFIX_OF_THREAD+"获取子节点中，排在我前面的"+waitPath);
                try{
                    zk.getData(waitPath, true, new Stat());//判断Watch的Znode有数据,注册Watcher
                    return false;
                }catch(KeeperException e){
                    if(zk.exists(waitPath,false) == null){//如果数据不存在，则注册Watcher
